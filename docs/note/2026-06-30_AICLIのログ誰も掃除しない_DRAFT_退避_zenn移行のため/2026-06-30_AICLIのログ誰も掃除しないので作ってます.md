# AI CLI のセッションログ、誰も掃除しないので作ってます

【画像①｜ヒーロー（記事トップ）｜2026-06-30_ai-log-clean_hero.png｜ここに差し込み、この行は削除】

【画像②｜記事の要約（インフォグラフィック）｜2026-06-30_ai-log-clean_infographic.png｜ここに差し込み、この行は削除】
...
問題は、放置していると延々と貯まり続けることでした。本体に retention 設定があるのは Claude Code くらいで、Codex CLI も Copilot CLI も Cursor Agent も opencode も Grok Build も、入れた瞬間から書き残し始めて、消す仕掛けは持っていません。HDD の片隅で静かに膨張するタイプの太り方です。
...
【画像③｜各 AI CLI のセッションログ容量実測｜2026-06-30_ai-log-clean_fig1.png｜ここに差し込み、この行は削除】
...
【画像④｜Antigravity の 3 系統セッション保存構造｜2026-06-30_ai-log-clean_fig2.png｜ここに差し込み、この行は削除】
...
問題はそこではなく、`settings.json` を覗いても retention・TTL・maxConversations のキー枠すら無いことです。公式 docs のどこにも自動掃除の設定は出てきません。`/switch` のピッカーで `Ctrl+Delete` を押して 1 件ずつ手で消す、それだけが用意された掃除手段でした。
...
#Claude #ClaudeCode #生成AI #AIエージェント #プロンプト #Anthropic #エンジニア #プログラミング #個人開発 #AI #Antigravity #Codex #OSS
